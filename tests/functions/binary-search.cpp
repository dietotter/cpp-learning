...
int binarySearch(const int *array, int target, int min, int max)
{
    ...
    while (min <= max)
    {
        ...
        int currentElem{ array[currentIndex] };
...
int binarySearchRecursive(const int *array, int target, int min, int max)
{
    ...
    if (currentElem > target)
    {
        return binarySearchRecursive(array, target, min, currentIndex - 1);
    }
    else if (currentElem < target)
    {
        return binarySearchRecursive(array, target, currentIndex + 1, max);
    }
    else
    {
        return currentIndex;
